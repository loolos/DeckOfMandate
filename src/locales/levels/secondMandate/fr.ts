...
export const messagesFrSecondMandate: Record<keyof typeof messagesEnSecondMandate, string> = {
  ...
  "event.nineYearsWar.desc":
    "À partir de 1688, la Guerre de Neuf Ans transforme la coalition de la Ligue d'Augsbourg en conflit prolongé sur la définition même de l'équilibre européen. La France de Louis XIV affronte un théâtre unique mais relié : places fortes rhénanes, Pays-Bas, blocus maritimes, commerce atlantique et positions coloniales s'influencent mutuellement. Les succès militaires existent, mais restent souvent temporaires ; à long terme, crédit, fiscalité et capacité administrative pèsent plus qu'une bataille isolée. En jeu, c'est une crise Continue : vous pouvez lancer des campagnes à chaque tour pour un coût indexé sur l'Alerte Europe, tandis que l'inaction accumule pression de légitimité et charge budgétaire.",
