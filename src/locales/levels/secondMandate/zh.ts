...
export const messagesZhSecondMandate = {
  ...
  "status.antiFrenchSentiment.history":
    "1670 年代后，法荷战争与反法同盟外交令欧洲对法政策迅速趋于协同，法国国力越高，外部抬价与牵制越强。机制上：当“权力+财政”大于 20 时，所有与「欧洲警觉」相关且需经费解决的事件立刻 +1 费用；其后该总和每再增加满 5 点，费用再 +1。且该状态存在期间，每回合结束都会向抽牌堆加入 1 张「反法遏制」（抽到时 50% 降 1 点权力或 1 点合法性；打出费用为 floor(欧洲警觉进度/2)；打出后直接消除不进弃牌堆）。回到 20 或以下时该状态移除。",
  ...
  "event.leagueOfAugsburg.desc":
    "1680 年代后，法国在莱茵边境与低地的扩张引发周边长期恐慌。1686 年，神圣罗马帝国诸侯、哈布斯堡与德意志诸邦在奥格斯堡结盟，随后英格兰、荷兰与西班牙也逐步向反法协同靠拢。对路易十四而言，这不只是一次外交摩擦，而是“欧洲均势”开始以制度化同盟约束法国。机制上：这是一个需要累计处理多次的持续外交压力；若当回合放任不管，你会承受与欧洲警觉相关的维护代价。",
  "event.nineYearsWar.name": "九年战争",
  "event.nineYearsWar.desc":
    "该事件带「持续」标签。每回合可支付「floor(欧洲警觉进度/2)，且至少为 1」经费（或干预）发动一次战役：1/9 概率决定性取胜并彻底结束事件；4/9 概率僵持（无效果）；4/9 概率取得局部战果（合法性 +1）。若本回合未处理，则发生局部失败（合法性 -1），但事件仍会保留。只要该事件在回合末仍存在，就会向抽牌堆加入 1 张「财政负担」。",
  ...
  "event.localWar.desc":
    "17 世纪后期的法兰西经常陷入“大战之间的小战”：莱茵边境、意大利北部与低地周边不断出现围城、突袭与补给线争夺。这类冲突往往规模有限，却会反复消耗财政、拖累军政节奏，并被列强当作测试法国意志的前哨战。机制上：该事件不能用「干预」直接解决；你需在军事冒险与政治让步之间二选一，且若拖到回合结束，下一回合的经费收入会受到惩罚。",
