...
export const messagesFrSecondMandate: Record<keyof typeof messagesEnSecondMandate, string> = {
  ...
  "event.revocationNantes.desc":
    "En 1598, Henri IV publia l'édit de Nantes pour mettre fin aux guerres de Religion : le catholicisme demeurait la foi d'État, tandis que les protestants français (huguenots) recevaient des droits de culte limités, des protections civiles et des garanties de sécurité. La couronne le révoque désormais. C'est un événement Continu : si ignoré, vous piochez 2 cartes de moins à chaque tour et il reste sur le plateau. Résolvez en choisissant une politique : Tolérance religieuse (Légitimité -1 immédiat ; gagnez le statut permanent Tolérance religieuse, qui effectue chaque tour un seul jet et injecte au plus un de trois risques confessionnels — controverse janséniste (15%), polémique arminienne (15%) ou agitation huguenote clandestine (15%), soit 45% au total ; le jet est ignoré si l'un des trois est déjà sur le plateau ou si aucun emplacement n'est libre ; à noter : résoudre la Polémique arminienne ou l'Agitation huguenote clandestine, au coût de 1 Financement chacune, ajoute aussi 1 carte Tension religieuse à la pioche — coût 2, purgée du deck à l'activation au lieu d'être défaussée), ou Répression sévère (pas de variation numérique immédiate ; gagnez Contenir les restes huguenots 3 et ajoutez 3 cartes « Réprimer les huguenots » au deck, coût 3 chacune ; chaque activation réduit les restes de 1 ; à 0, retirez le statut et purgez ces cartes de la main/du deck/de la défausse). Tant que ce statut reste actif, tous les deux tours (à la fin de la phase d'événements) une résurgence huguenote se déclenche : 1 carte « Réprimer les huguenots » supplémentaire est ajoutée au deck et les restes +1, reflétant des réseaux clandestins qui se reconstruisent plus vite que la répression ne peut s'achever. Tant que ce statut de containment existe, la victoire du Chapitre 2 est bloquée.",
