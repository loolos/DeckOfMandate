...
export const messagesZhSecondMandate = {
  ...
  "event.revocationNantes.desc":
    "1598 年亨利四世颁布《南特敕令》，原为在宗教战争后恢复秩序：在保持天主教国教地位的前提下，给予法国新教徒（胡格诺派）有限礼拜自由、民事权利与若干安全保障。如今王权决定撤销它。此事件带「持续」标签：若不处理，将每回合额外少抽 2 张，且事件会保留在列表中。处理时二选一：\n- 宗教宽容：立刻合法性 -1，并获得永久状态「宗教宽容」。该状态每回合在事件相位末进行一次掷骰，至多注入三类宗派事件之一：詹森派争议（15%）、阿米尼乌斯派争论（15%）、胡格诺地下网络骚动（15%），合计 45% 概率触发其中一种；若三者已有任意一个在场或当前没有空槽，则跳过本次注入。注意：处理「阿米尼乌斯派争论」或「胡格诺地下网络骚动」（费用各 1 经费）会向抽牌堆加入 1 张「宗教矛盾」额外卡牌（费用 2，打出后从牌库移除而非进入弃牌堆）。\n- 严厉镇压：无立刻数值变化，但获得状态「遏制胡格诺-剩余势力 3」，并向抽牌堆加入 3 张临时牌「镇压胡格诺派」（费用 3）。每打出 1 张，「剩余势力」-1；降到 0 时，移除该状态并清除抽牌堆/弃牌堆/手牌中的全部「镇压胡格诺派」。同时只要该状态仍在场，每两个回合（在事件相位末）会有一次「胡格诺残余卷土重来」：向抽牌堆加入 1 张「镇压胡格诺派」并使「剩余势力」+1，反映地下网络持续重建、镇压战线无法快速收束。注意：该状态存在时，第二关无法胜利。",
