...
describe("resolveEndOfYearPenalties", () => {
  ...
  it("noble resentment unresolved reduces power by 1 and adds noble grudge for 2 turns", () => {
    const base = createInitialState(5_006, "secondMandate");
    const s0 = {
      ...base,
      resources: { ...base.resources, power: 4 },
      slots: {
        ...EMPTY_EVENT_SLOTS,
        A: { instanceId: "e_noble", templateId: "nobleResentment" as const, resolved: false },
      },
    };
    const s1 = resolveEndOfYearPenalties(s0);
    expect(s1.resources.power).toBe(3);
    const grudge = s1.playerStatuses.find((st) => st.templateId === "nobleGrudge");
    expect(grudge).toBeTruthy();
    expect(grudge?.turnsRemaining).toBe(2);
  });
