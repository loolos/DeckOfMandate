...
describe("desiredProceduralEventCountWhenAllEmpty", () => {
  ...
  it("forces first-mandate turn 2 all-empty procedural refill to exactly 1", () => {
    const s0 = createInitialState(101_006, "firstMandate");
    const state: GameState = {
      ...s0,
      levelId: "firstMandate",
      turn: 2,
    ...
    expect(desiredProceduralEventCountWhenAllEmpty(state, 0.0)).toBe(1);
    expect(desiredProceduralEventCountWhenAllEmpty(state, 0.95)).toBe(1);
  });
});
...
describe("beginYear + playerStatuses", () => {
  ...
  it("standalone second-mandate year-1 opening places fixed A/B and a procedural event in C", () => {
    ...
    expect(occupiedSlots).toHaveLength(3);
  });
  ...
  it("retention-capacity status also raises draw overflow cap during beginYear", () => {
    const started = createInitialState(55_783, "thirdMandate");
    const cardsById: Record<string, CardInstance> = {
      h0: { instanceId: "h0", templateId: "funding" },
      h1: { instanceId: "h1", templateId: "funding" },
      h2: { instanceId: "h2", templateId: "funding" },
