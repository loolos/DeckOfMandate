...
function writeAction(w: ByteWriter, a: GameAction): void {
  switch (a.type) {
    ...
    case "PICK_LOUIS_XIV_LEGACY":
      w.pushU8(ACTION_TAG.PICK_LOUIS_XIV_LEGACY);
      w.pushU8(slotToByte(a.slot));
      w.pushU8(a.directRule ? 1 : 0);
      return;
...
function readAction(r: ByteReader, currentHand: readonly string[]): GameAction {
  ...
  switch (tag) {
    ...
    case ACTION_TAG.PICK_LOUIS_XIV_LEGACY:
      return { type: "PICK_LOUIS_XIV_LEGACY", slot: byteToSlot(r.readU8()), directRule: r.readU8() !== 0 };
...
function readRunRecordV4(r: ByteReader): {
  ...
  if (mode === "continuity") {
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      const templateId = readUtf8LevelId(r) as import("../../types/card").CardTemplateId;
