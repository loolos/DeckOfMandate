/**
 * Runs a larger second->third campaign batch under Vitest (Vite resolves level glob).
 * Usage: node scripts/runSecondToThirdCampaignBatch.mjs [seedStart] [runCount]
...
const testFile = "src/levels/sunking/sim/secondToThirdCampaignLongBatch.test.ts";
...
const run = spawnSync(
  ...
  [vitestCliPath, "run", testFile, "--testNamePattern", "large batch"],
  {
    ...
    env: {
      ...process.env,
      VITEST_LONG_SECOND_TO_THIRD: "1",
      VITEST_SECOND_TO_THIRD_BATCH_SEED: String(seedStart),
      VITEST_SECOND_TO_THIRD_BATCH_RUNS: String(runCount),
    },
  },
);
