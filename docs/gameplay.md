...
- **Anti-French Sentiment status**: if `power + treasuryStat > 20`, apply status marker; while active, every full `+5` overflow above 20 increases all funding-based event solve costs by `+1` (removed once `power + treasuryStat <= 20`).
